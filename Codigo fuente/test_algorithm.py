...
for index in range(0, nimages):
	img = cv2.imread("images_test_svm/"+str(index)+".jpg", 0)
	...
	for npaContour in npaContours: 
		...
		if(radius > (nmode-10) and radius < (nmode+10)):
			...
			Image.fromarray(separeted_char).convert("RGB").save("./process/separated_chars/"+str(cont)+".jpg")
			...
			cont += 1
	word_size = len(manual_results[index])
	right_answers = 0
	platePrediction += "----"
	for c in range(0, word_size):
		if(platePrediction[c] == manual_results[index][c]):
			right_answers += 1

	accuracy_per_word = right_answers/word_size		
	print("Prediction: ", platePrediction, "Accuracy", accuracy_per_word)
	print("Manual result", manual_results[index])
	print("------------------------------------")
	Image.fromarray(aux).convert("RGB").save("./process/segmented_plates/"+str(index)+".jpg")
	cumulative_accuracy += accuracy_per_word

print("Mean Accuracy", cumulative_accuracy/nimages)
